...
Rerun wgcna with gsnap DESEq2 genes
...
order=names(data)[c(1:2,33,3:32)]
cdata = select(data, all_of(order))

write_delim(cdata, "deseq_wgcna.txt", delim="\t")

temp<-cdata %>% 
  subset(wgcna=="yellow" | wgcna=="blue") %>%
  pivot_longer(cols = c(10:33))
